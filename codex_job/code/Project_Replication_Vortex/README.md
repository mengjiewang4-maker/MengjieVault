...
按文件修改时间看，最新一批是在 `2026-04-20 18:03` 生成的 6 个文件，来源于 `disclination_vortex_gds.py`。

| 文件 | R | a (um) | r_ratio | r_hole (um) | 说明 |
| --- | ---: | ---: | ---: | ---: | --- |
| `output/wmj20260420disc_R60_a0p554_r0p111.gds` | 60 | 0.554 | 0.20 | 0.1108，文件名四舍五入为 0.111 | 大尺寸版本 |
| `output/wmj20260420disc_R30_a0p554_r0p111.gds` | 30 | 0.554 | 0.20 | 0.1108，文件名四舍五入为 0.111 | 半径减半版本 |
| `output/wmj20260420disc_R60_a0p559_r0p112.gds` | 60 | 0.559 | 0.20 | 0.1118，文件名四舍五入为 0.112 | 大尺寸版本 |
| `output/wmj20260420disc_R30_a0p559_r0p112.gds` | 30 | 0.559 | 0.20 | 0.1118，文件名四舍五入为 0.112 | 半径减半版本 |
| `output/wmj20260420disc_R60_a0p559_r0p084.gds` | 60 | 0.559 | 0.15 | 0.08385，文件名四舍五入为 0.084 | 大尺寸、小孔版本 |
| `output/wmj20260420disc_R30_a0p559_r0p084.gds` | 30 | 0.559 | 0.15 | 0.08385，文件名四舍五入为 0.084 | 半径减半、小孔版本 |

这一批参数单位统一按 um 记录。脚本设置了 `ly.dbu = 0.001`，在 KLayout 中表示 1 个数据库单位是 0.001 um。
...
```text
output/wmjYYYYMMDDdisc_R{R}_a{a_token}_r{r_token}.gds
```

其中：

- `a_token` 是 `a` 保留 3 位小数后把小数点替换成 `p`，例如 `0.559` 变成 `0p559`。
- `r_token` 是实际孔半径 `r_hole` 保留 3 位小数后把小数点替换成 `p`，例如 `0.1118` 变成 `0p112`。
- 每组参数会同时生成 `R = 60` 和 `R = 30` 两个版本。
...
- 最新这批 `disc_R...` 文件是圆形空气孔 disclination vortex；早期 `vortex230/vortex260` 文件来自旧命名和旧流程，不要和这批参数混用。
