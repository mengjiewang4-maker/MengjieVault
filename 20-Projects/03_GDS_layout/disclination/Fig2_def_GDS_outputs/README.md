...
```bash
MPLCONFIGDIR=/private/tmp/matplotlib /Users/mac/miniconda3/bin/python3 scripts/disclination_fig2_def_gds.py --lattice-constant-nm 500 --hole-radius-ratio 0.20 --center-to-hole-ratio 0.45 --lattice-range 13
```
...
- 通过 90° C4 扇区压缩到 72° 并复制 5 份实现 added 1/4 sector C5 结构。
...
- 论文 Extended Data Fig. 3 给出 db/di，但没有完整说明每个边界孔的逐点位移规则；当前 GDS 仅显式应用核心 dc 修正，db/di 写入参数等待进一步核对。
